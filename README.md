...
- Optional PNG image assets (`type: "image"`) can be loaded from local filesystem paths (`image_path`) via LVGL’s lodepng decoder for static blits (no remote downloads). (`main.c`, `lv_conf.h`)
- Optional MSP glyph atlas blitting can render Betaflight-style PNG grids onto the OSD via the `glyphs` UDP payload when enabled in config. (`main.c`, `msp_glyph_atlas.c`)
...
- Image assets (`type: "image"`) load a PNG from `image_path` (absolute paths are allowed and normalized to the LVGL stdio driver) and place it at `x`,`y`. `width`/`height` are optional for sizing the widget’s box; omit them to use the PNG’s native dimensions.
- Set `WAYBEAM_IMAGE_DEBUG_RECT=1` to draw an opaque magenta rectangle at the image position/size, which helps confirm the LVGL draw/flush path when troubleshooting invisible PNGs.
- Glyph atlas config: enable with `glyphs_enabled`, set `glyph_atlas_path`, and optionally override `glyph_grid_cols`, `glyph_grid_rows`, `glyph_page_count`, or the `glyph_origin_x`/`glyph_origin_y` grid origin. Use the UDP `glyphs` payload (see `CONTRACT.md`) to blit glyph IDs at grid row/col positions.
...
Example image asset entry:
```json
{ "type": "image", "id": 2, "enabled": true, "image_path": "/etc/waybeam_osd/logo.png", "x": 40, "y": 40 }
```
