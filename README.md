...
- Encoder telemetry is pulled best-effort from an already loaded `libmi_venc.so` (discovered via `dlsym(RTLD_DEFAULT, …)`). If `WAYBEAM_VENC_FORCE_LOAD=1` is set and the library is not preloaded, the OSD will try to `dlopen` it (with `RTLD_NODELETE` when available) after warning that doing so could hang the encoder on exit; when we have to `dlopen`, shutdown skips shared-library destructors via `_exit()` to avoid tripping encoder finalizers. (`main.c`)
- UDP listener on port `7777` consumes JSON payloads documented in `CONTRACT.md` (`values[]` + optional `texts[]`). The socket is fully drained whenever readable, sparse updates are supported via `null` entries, and coalesced results refresh the screen no faster than every 32 ms (~30 fps). `idle_ms` (default 100 ms, clamped 10–1000) still caps the wait when no new data arrives. (`main.c`, `CONTRACT.md`)
