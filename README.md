...
- Image assets (`type: "image"`) load a PNG from `image_path` (absolute or relative to the working directory) and place it at `x`,`y`. `width`/`height` are optional for sizing the widget’s box; omit them to use the PNG’s native dimensions.
