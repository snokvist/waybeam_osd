...
- Radio stats can be pulled directly from local control sockets without invoking external tools: `--hostapd <[iface,]sta-mac>` issues a `STA <mac>` command against `/run|/var/run/hostapd` sockets (auto-picks an interface when omitted), and `--wpa-cli <iface>` issues `SIGNAL_POLL` against `/run|/var/run/wpa_supplicant/<iface>`. Parsed keys (e.g., `signal`, `rx_packets`, `tx_packets`, `RSSI`, `LINKSPEED`) are merged on top of loaded INI data so `@key` references resolve to the freshest command output; missing or failed commands collapse to `null` placeholders instead of reusing stale values.
- `watch` refreshes both INI files and command outputs each interval so changing RSSI or packet counters propagate automatically. Example: `./waybeam send --ini /tmp/radio.ini --hostapd wlan0,aa:bb:cc:dd:ee:ff --wpa-cli wlan0 --values "0=@signal,1=@RSSI" --texts "0=@tx_packets,1=@rx_packets"`.
