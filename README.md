...
- Encoder telemetry is pulled best-effort from an already loaded `libmi_venc.so` (discovered via `dlsym(RTLD_DEFAULT, …)`), and an explicit load only succeeds when the library is already resident/`RTLD_NOLOAD`-visible. If the library is not resident, stats continue without encoder metrics even when `WAYBEAM_VENC_FORCE_LOAD=1`, to avoid encoder teardown on exit. (`main.c`)
