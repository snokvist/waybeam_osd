...
- Image assets (`type: "image"`) render a local image file from `image_path` (LVGL fs driver prefixes like `"A:/path.png"` are accepted) at `x`/`y`. Image and glyph sizes always use the native PNG pixels (no resizing). To use a Betaflight-style glyph atlas, append `#atlas` (or `#glyphs`) to `image_path` and drive the glyph ID via `value_index` (the atlas is split into 16x16 tiles per page, pages stacked vertically, and glyph size is derived from the PNG dimensions). Atlas glyphs are cached and reused to avoid reallocating memory every frame; glyph decoding streams rows from disk to keep memory lower. Image assets still honor `background` and `background_opacity` for tinting the image object, but ignore bar/text-only fields like `label` or `text_index`.
