...
- Transparent LVGL OSD that renders up to 8 configurable assets (basic bar plus `lv_example_bar_2` and `lv_example_scale_10`-styled) defined in `config.json` and driven by a UDP payload. Optional descriptors live to the right of bars (static `label` or live `texts[]` channel). Background stays fully transparent; assets refresh at the configured cadence. (`main.c`, `config.json`)
- UDP listener on port `7777` consumes JSON payloads documented in `CONTRACT.md` (`values[]` + optional `texts[]`). The socket is drained each frame so only the latest datagram drives the screen. Refresh is scheduled by `refresh_ms` (default 100 ms, clamped 10–1000). (`main.c`, `CONTRACT.md`)
