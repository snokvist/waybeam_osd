...
- Single stats widget in the top-left (gated by `show_stats`) shows OSD/display resolution, asset count, FPS, timing, and live system value/text banks. When `udp_stats` is true, it also lists the UDP numeric/text banks vertically to avoid width overflow. (`main.c`, `config.json`)
