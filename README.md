...
- Encoder telemetry is loaded dynamically from `libmi_venc.so` (kept resident with `RTLD_NODELETE` when available) so querying FPS/bitrate won’t tear down the running encoder when the OSD exits; if the library is missing, stats continue without encoder metrics. (`main.c`)
