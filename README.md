...
- `fit_to_content` (default `true`) shrinks the LVGL/RGN canvas to the bounding box of enabled assets and the stats widget plus `fit_pad` pixels of padding, and subtracts that offset from asset coordinates so their on-screen position stays anchored. `osd_x`/`osd_y` offset the resulting region inside the video frame; disable `fit_to_content` to force the configured `width`/`height` instead.
