...
- Optional PNG image assets (`type: "image"`) can be loaded from local filesystem paths (`image_path`) for static blits (no remote downloads). Images are decoded via lodepng into malloc-backed buffers and rendered at native size. (`main.c`)
...
- Image assets (`type: "image"`) load a PNG from `image_path` (absolute paths are allowed) and place it at `x`,`y` using the PNG’s native dimensions.
