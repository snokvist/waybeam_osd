...
- Encoder telemetry is pulled best-effort from an already loaded `libmi_venc.so` (discovered via `dlsym(RTLD_DEFAULT, …)`), and an explicit load uses `RTLD_NODELETE` to avoid tearing down the encoder on exit. If the library is not resident, stats continue without encoder metrics unless `WAYBEAM_VENC_FORCE_LOAD=1` is set to allow an explicit load. (`main.c`)
