...
- Optional PNG image assets (`type: "image"`) can be loaded from local filesystem paths (`image_path`) and rendered at their native dimensions for static blits (no remote downloads). (`main.c`, `lv_conf.h`)
...
- Image assets (`type: "image"`) load a PNG from `image_path` (absolute paths are allowed and normalized to the LVGL stdio driver) and place it at `x`,`y`. Image assets always use the PNG’s native dimensions; `width`/`height` are ignored.
