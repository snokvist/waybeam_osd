...
- `realtime_flip` optionally enables SigmaStar `setRealtimeFlip` for the attached OSD VPE channel at startup (default `false`).
...
- Image assets (`type: "image"`) render a local image file from `image_path` (LVGL fs driver prefixes like `"A:/path.png"` are accepted) at `x`/`y`; optional `width`/`height` set a size override, otherwise the native image size is used. Image assets still honor `background` and `background_opacity` for tinting the image object, but ignore bar/text-only fields like `label` or `text_index`.
