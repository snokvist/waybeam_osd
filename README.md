...
- Text assets also support `scale_pct` (25–400, default 100) via LVGL transform zoom, which lets you enlarge text without bundling additional larger font files. For scaled text/image assets, `orientation: "left"` anchors the right edge (pivot + placement) while `"right"` anchors the left edge.
