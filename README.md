...
- OSD canvas auto-sizes to the visible assets + stats overlay with a configurable padding halo, then sets the MI_RGN display offset to keep absolute placement intact while shrinking the LVGL display. The flush path converts ARGB8888 -> ARGB1555 and only calls `MI_RGN_UpdateCanvas` once per LVGL tick; a second ARGB1555 RGN layer is reserved for pre-converted icons blitted via row copies. (`main.c`, `config.json`)
...
## Icon layer
- A second OSD region (layer 1) stays reserved for icons that are pre-converted to ARGB1555. Copy rows into it with
  `blit_icon_argb1555()` (fast memcpy per row) and update the canvas once per icon change. The default icon surface is 256x64 at
