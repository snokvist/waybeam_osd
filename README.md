...
- Encoder telemetry is pulled best-effort from a preloaded `libmi_venc.so` (opened with `RTLD_NOLOAD` + `RTLD_NODELETE` when available) so querying FPS/bitrate won’t disturb the running encoder on exit; if the library is not already loaded, stats continue without encoder metrics unless `WAYBEAM_VENC_FORCE_LOAD=1` is set to allow an explicit load. (`main.c`)
