...
typedef struct {
    ...
    anim_frame_t *anim_frames;
    int frame_count;
    int current_frame;
    lv_timer_t *anim_timer;
    lv_image_dsc_t anim_dsc;
} asset_t;
...
static void parse_assets_array(const char *json)
{
    ...
    while (*p && asset_count < MAX_ASSETS) {
        ...
        if (json_get_string_range(obj_start, obj_end, "type", type_buf, sizeof(type_buf)) == 0) {
            ...
            } else if (strcmp(type_buf, "image") == 0) { } else if (strcmp(type_buf, "animation") == 0) { a.cfg.type = ASSET_ANIMATION;
        ...
        json_get_string_range(obj_start, obj_end, "anim_path", a.cfg.anim_path, sizeof(a.cfg.anim_path));
        if (json_get_int_range(obj_start, obj_end, "fps", &v) == 0) a.cfg.fps = v;
        char orient_buf[16];
...
static void parse_udp_asset_updates(const char *buf)
{
    ...
    while (*p) {
        ...
        if (json_get_string_range(obj_start, obj_end, "type", type_buf, sizeof(type_buf)) == 0) {
            ...
            if (strcmp(type_buf, "text") == 0) {
                new_type = ASSET_TEXT;
            } else if (strcmp(type_buf, "image") == 0) { } else if (strcmp(type_buf, "animation") == 0) { new_type = ASSET_ANIMATION;
...
{
    if (!asset) return;
    if (asset->anim_timer) { lv_timer_del(asset->anim_timer); asset->anim_timer = NULL; }
    if (asset->anim_frames) { for(int i=0; i<asset->frame_count; i++) free(asset->anim_frames[i].data); free(asset->anim_frames); asset->anim_frames = NULL; asset->frame_count = 0; }
...
// Animation functions inserted here
static int compare_filenames(const void *a, const void *b) {
    return strcmp(*(const char **)a, *(const char **)b);
}

static void load_animation_frames(asset_t *asset) {
    if (!asset || asset->cfg.anim_path[0] == '\0') return;
    if (asset->anim_frames) return;

    DIR *d;
    struct dirent *dir;
    d = opendir(asset->cfg.anim_path);
    if (!d) {
        fprintf(stderr, "Animation: Failed to open dir %s\n", asset->cfg.anim_path);
        return;
    }
    ...
    while ((dir = readdir(d)) != NULL) {
        if (dir->d_type == DT_REG) {
            ...
            if (ext && strcasecmp(ext, ".png") == 0) count++;
        }
    }
    rewinddir(d);
    ...
    asset->anim_frames = (anim_frame_t *)calloc(count, sizeof(anim_frame_t));
    if (!asset->anim_frames) {
        closedir(d);
        return;
    }
    ...
    int found = 0;
    while ((dir = readdir(d)) != NULL && found < count) {
        if (dir->d_type == DT_REG) {
            const char *ext = strrchr(dir->d_name, '.');
            if (ext && strcasecmp(ext, ".png") == 0) {
                filenames[found] = strdup(dir->d_name);
                found++;
            }
        }
    }
    ...
    qsort(filenames, found, sizeof(char *), compare_filenames);

    size_t total_mem = 0;
    size_t limit = 4 * 1024 * 1024; // 4MB cap
    int loaded = 0;

    for (int i = 0; i < found; i++) {
        if (total_mem >= limit) {
            fprintf(stderr, "Animation: Memory limit reached (%zu bytes). Loaded %d/%d frames.\n", total_mem, loaded, found);
            free(filenames[i]);
            continue;
        }

        char fullpath[512];
        snprintf(fullpath, sizeof(fullpath), "%s/%s", asset->cfg.anim_path, filenames[i]);

        char *data = NULL;
        size_t sz = 0;
        if (read_file(fullpath, &data, &sz) == 0) {
            asset->anim_frames[loaded].data = (uint8_t *)data;
            asset->anim_frames[loaded].size = sz;
            total_mem += sz;
            loaded++;
            fprintf(stderr, "Animation: Loaded %s (%zu bytes)\n", filenames[i], sz);
        } else {
            fprintf(stderr, "Animation: Failed to load %s\n", fullpath);
        }
        free(filenames[i]);
    }
    free(filenames);

    asset->frame_count = loaded;
    asset->current_frame = 0;
    fprintf(stderr, "Animation: Asset %d loaded %d frames. Total RAM: %zu bytes.\n", asset->cfg.id, loaded, total_mem);
}

static void anim_timer_cb(lv_timer_t *timer) {
    asset_t *asset = (asset_t *)lv_timer_get_user_data(timer);
    if (!asset || !asset->obj || asset->frame_count == 0) return;

    asset->current_frame = (asset->current_frame + 1) % asset->frame_count;

    anim_frame_t *frame = &asset->anim_frames[asset->current_frame];

    lv_image_dsc_t *dsc = &asset->anim_dsc;
    memset(dsc, 0, sizeof(lv_image_dsc_t));
    dsc->header.magic = LV_IMAGE_HEADER_MAGIC;
    dsc->header.cf = LV_COLOR_FORMAT_ARGB8888;
    dsc->header.flags = 0;
    dsc->data = frame->data;
    dsc->data_size = frame->size;

    lv_image_set_src(asset->obj, dsc);
}
static void create_asset_visual(asset_t *asset)
{
    ...
    switch (asset->cfg.type) {
        ...
        case ASSET_IMAGE:
            ...
            break;
        case ASSET_ANIMATION:
            load_animation_frames(asset);
            asset->obj = create_image_asset(asset);
            if (asset->obj && asset->frame_count > 0) {
               int interval = asset->cfg.fps > 0 ? 1000 / asset->cfg.fps : 100;
               asset->anim_timer = lv_timer_create(anim_timer_cb, interval, asset);
            }
            break;
        default:
            asset->obj = create_bar(asset);
