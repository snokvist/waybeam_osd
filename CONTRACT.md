...
- Cleanup troubleshooting: if device firmware intermittently stalls inside `MI_RGN_DeInit`, set environment variable `LVGLTEST_SKIP_RGN_DEINIT=1` before launching. The process will still detach/destroy the region but will skip the driver deinit call during shutdown to avoid encoder hangs; use this only for environments known to wedge in deinit.
