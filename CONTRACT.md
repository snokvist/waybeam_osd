...
- A second bank of 8 system values is always available on `value_index` slots `8-15` without going over UDP. System slots 8-11 are populated automatically with SoC temperature (`ipctool --temp`), CPU load (0–100), encoder FPS, and current encoder bitrate (queried best-effort from an already loaded `libmi_venc.so`, discovered via `dlsym(RTLD_DEFAULT, …)`. Setting `WAYBEAM_VENC_FORCE_LOAD=1` will only attach if the library is already resident/`RTLD_NOLOAD`-visible to avoid teardown on exit); slots 12-15 stay reserved for future system metrics.
