...
- `osd_x`, `osd_y` (int, optional): On-screen origin for the RGN. Default `0,0`.
  ...
  - Asset fields:
    ...
    - `image_path` (string, image only): local filesystem path to a PNG image (absolute paths are accepted); omit to hide the image. Image assets always use the PNG’s native dimensions.
    ...
    - `width`, `height` (int): size in pixels for bars/text. For text, enables wrapping.
