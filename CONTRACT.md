...
- The UDP socket is drained each frame so only the latest datagram drives the screen; older queued packets are discarded. Send at or below the configured `refresh_ms` rate to avoid waste.
